...
FROM productos AS p
LEFT JOIN detalle_ventas AS dt ON p.id_producto = dt.id_producto
...
SELECT 
    ...
    ROUND(AVG(dt.precio_unitario),2) AS precio_promedio
FROM productos AS p
INNER JOIN detalle_ventas AS dt ON p.id_producto = dt.id_producto
GROUP BY p.id_producto;
...
LEFT JOIN productos AS p ON pr.id_proveedor = p.id_proveedor
...
INNER JOIN productos AS p ON cat.id_categoria = p.id_categoria
...
ORDER BY e.nombre, e.apellido, p.nombre;
...
FROM categorias AS cat
JOIN productos AS  p ON cat.id_categoria = p.id_categoria
JOIN detalle_ventas AS dt ON p.id_producto = dt.id_producto
...
SELECT 
    ...
    dt.cantidad
FROM ventas AS v
JOIN detalle_ventas AS dt ON v.id_venta = dt.id_venta;
...
SELECT 
	e.nombre,
    e.apellido,
    COUNT(v.id_venta) AS cantidad_ventas_posteriores
FROM empleados AS e
LEFT JOIN ventas AS v ON v.fecha >= e.fecha_ingreso
